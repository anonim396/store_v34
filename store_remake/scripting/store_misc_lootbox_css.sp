		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (g_iLootboxEntityRef[i] != INVALID_ENT_REFERENCE)
			{
				Store_GiveItem(i, g_iItemID[g_iClientBox[i]], 0, 0, 0);
					...
					MC_PrintToChat(i, "%s%t", g_sChatPrefix, "You haven't opend the box in given time");
					if (!CM_IsClientModUser(i))
					chatm.CPrintToChat(i, "%s%t", g_sChatPrefix, "You haven't opend the box in given time");
					#else
...
public int Lootbox_Equip(int client, int itemid)
{
	...
	{
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox round ended");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox round ended");
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox round ended");
	...
	{
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox map ended");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox map ended");
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox map ended");
	...
	{
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox case is opening");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox case is opening");
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Lootbox case is opening");
...
public Action Timer_Open(Handle timer, int client)
{
	...
	if (itemid == -1)
	{
		...
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
				C_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
				#else
				#if defined _chat_modern_included
					chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
					#else
					PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, item back. Inform admin log");
				#endif
			#endif
		#endif
	...
	if (Store_HasClientItem(client, itemid))
	{
		if (g_iPriceBack[g_iClientBox[client]] <= 0)
		{
			Store_GiveItem(client, g_iItemID[g_iClientBox[client]], 0, 0, 0);
			#if defined _clientmod_included && defined _chat_modern_included
				MC_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
				if (!CM_IsClientModUser(client))
					chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
				#else
				#if defined _clientmod_included
					MC_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
					C_PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
					#else
					#if defined _chat_modern_included
						chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
						#else
						PrintToChat(client, "%s%s", g_sChatPrefix, "Error occured, no price back. Inform admin log");
					#endif
				#endif
			#endif
		}
		else
		{
			...
			#if defined _clientmod_included && defined _chat_modern_included
				MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
				if (!CM_IsClientModUser(client))
					chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
				#else
				#if defined _clientmod_included
					MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
					C_PrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
					#else
					#if defined _chat_modern_included
						chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
						#else
						PrintToChat(client, "%s%t", g_sChatPrefix, "Already own item from box. Get Credits price back", item.szName, handler.szType, RoundFloat(g_iPriceBack[g_iClientBox[client]]*view_as<float>(g_iSellRatio[g_iClientBox[client]])), g_sCreditsName);
					#endif
				#endif
			#endif
			if (g_iClientLevel[client] == LEVEL_RED)
			{
				...
				for (int i = 1; i <= MaxClients; i++)
				{
					MC_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item red", name, item.szName, handler.szType);
					if (!CM_IsClientModUser(i))
						chatm.CPrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item red", name, item.szName, handler.szType);
				}
				#else
					#if defined _clientmod_included
						for (int i = 1; i <= MaxClients; i++)
						{
							MC_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item red", name, item.szName, handler.szType);
							C_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item red", name, item.szName, handler.szType);
						}
					#else
						#if defined _chat_modern_included
							chatm.CPrintToChatAll("%s%t", g_sChatPrefix, "Chat won lootbox item red", name, item.szName, handler.szType);
						#else
			...
			if (g_iClientLevel[client] == LEVEL_GOLD)
			{
				...
				for (int i = 1; i <= MaxClients; i++)
				{
					MC_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item gold", name, item.szName, handler.szType);
					if (!CM_IsClientModUser(i))
						chatm.CPrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item gold", name, item.szName, handler.szType);
				}
				#else
					#if defined _clientmod_included
						for (int i = 1; i <= MaxClients; i++)
						{
							MC_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item gold", name, item.szName, handler.szType);
							C_PrintToChat(i, "%s%t", g_sChatPrefix, "Chat won lootbox item gold", name, item.szName, handler.szType);
						}
					#else
						#if defined _chat_modern_included
							chatm.CPrintToChatAll("%s%t", g_sChatPrefix, "Chat won lootbox item gold", name, item.szName, handler.szType);
						#else
	...
	else
	{
		...
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
			if (!CM_IsClientModUser(client))
				chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
				C_PrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
				#else
				#if defined _chat_modern_included
					chatm.CPrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
					#else
					PrintToChat(client, "%s%s", g_sChatPrefix, sBuffer);
				#endif
			#endif
		#endif
