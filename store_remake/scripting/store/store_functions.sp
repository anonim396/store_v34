void StoreLogMessage(int client = 0, int level, const char[] message, any ...)
{
	...
	else if (g_eCvars[g_cvarPluginsLogging].aCache == 1)
	{
		...
		BuildPath(Path_SM, m_szPath, sizeof(m_szPath), "logs/store.log");

		Handle hFile = OpenFile(m_szPath, "a+");
		if (hFile == INVALID_HANDLE)
		{
			LogError("Failed to open log file: %s", m_szPath);
			return;
		}

		char logMessage[256];
		Format(logMessage, sizeof(logMessage), "%s - %L - %s", sLevel, client, sReason);

		WriteFileLine(hFile, logMessage);
		CloseHandle(hFile);
	}
}
...
void Store_LogMessage(int client,int credits, const char[] message,any ...)
{
	...
	if(g_eCvars[g_cvarLogging].aCache == 1)
	{
		...
		if (hFile == INVALID_HANDLE)
		{
			LogError("Failed to open registration file: %s", m_szPath);
			return;
		}

		char logMessage[256];
		Format(logMessage, sizeof(logMessage), "%N credits have changed by %d. Reason: %s", client, credits, m_szReason);
