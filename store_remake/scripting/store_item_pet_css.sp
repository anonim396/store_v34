...
Action Command_Hide(int client, int args)
{
	...
	if (g_bHide[client])
	{
			...
			MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Item hidden", "pet");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Item hidden", "pet");
			#else
	...
	{
		#if defined _clientmod_included && defined _chat_modern_included
			MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Item visible", "pet");
			if (!CM_IsClientModUser(client))
			chatm.CPrintToChat(client, "%s%t", g_sChatPrefix, "Item visible", "pet");
			#else
			#if defined _clientmod_included
				MC_PrintToChat(client, "%s%t", g_sChatPrefix, "Item visible", "pet");
